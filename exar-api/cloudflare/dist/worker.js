// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/compose.js
...
// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/request/constants.js
var GET_MATCH_RESULT = /* @__PURE__ */ Symbol();

// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/utils/buffer.js
...
// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/utils/body.js
...
// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/utils/url.js
var splitPath = (path) => {
...
// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/request.js
...
// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/utils/html.js
...
// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/context.js
...
// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/router.js
...
// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/utils/constants.js
var COMPOSED_HANDLER = "__COMPOSED_HANDLER";

// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/hono-base.js
...
// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/router/utils.js
var createNullObject = () => /* @__PURE__ */ Object.create(null);

// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/router/reg-exp-router/matcher.js
var emptyParam = [];
...
// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/router/reg-exp-router/node.js
...
// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/router/reg-exp-router/trie.js
var Trie = class {
  #context = { varIndex: 0 };
  #root = new Node();
  #index = 0;
...
// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/router/reg-exp-router/router.js
...
// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/router/smart-router/router.js
...
// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/router/trie-router/node.js
...
// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/router/trie-router/router.js
...
// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/hono.js
...
// ../lexmora/lexmora-api/cloudflare/node_modules/hono/dist/middleware/cors/index.js
var cors = (options) => {
  const opts = {
    origin: "*",
...
// exar-api/cloudflare/src/jalali.ts
var gdm = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334];
var jMonthLen = [31, 31, 31, 31, 31, 31, 30, 30, 30, 30, 30, 29];
...
function monthKeyFromGregorian(date) {
...
// exar-api/cloudflare/src/index.ts
var app = new Hono2();
app.use("*", async (c, next) => {
...
function validateShares(shares) {
  ...
  if (Math.abs(sum - 1) > 1e-3) return "shares must sum to 1";
  return null;
}
...
async function insertExpenseShares(db, expenseId, shares) {
  const stmts = shares.map(
    (s) => db.prepare(`INSERT INTO expense_shares (expense_id, person_id, share) VALUES (?, ?, ?)`).bind(expenseId, s.person_id, s.share)
  );
  if (stmts.length) await db.batch(stmts);
}
async function loadSharesForExpenses(db, expenseIDs) {
...
app.get("/api/persons", async (c) => {
  const rows = await c.env.DB.prepare(`SELECT id, name FROM persons ORDER BY id`).all();
  return c.json(rows.results || []);
});
app.get("/api/shops", async (c) => {
  const q = (c.req.query("q") || "").trim();
  if (q && [...q].length < 3) return c.json([]);
  const rows = q ? await c.env.DB.prepare(
    `SELECT id, name FROM shops WHERE name LIKE ? COLLATE NOCASE ORDER BY name`
  ).bind(`%${q}%`).all() : await c.env.DB.prepare(`SELECT id, name FROM shops ORDER BY name`).all();
  return c.json(rows.results || []);
});
...
app.delete("/api/shops/:id", async (c) => {
  const id = Number(c.req.param("id"));
  if (!Number.isFinite(id)) return jsonError("invalid shop id", 400);
  const inUse = await c.env.DB.prepare(`SELECT COUNT(*) AS c FROM expenses WHERE shop_id = ?`).bind(id).first();
...
app.get("/api/items", async (c) => {
  ...
  ).bind(`%${q}%`).all() : await c.env.DB.prepare(`SELECT id, name FROM items ORDER BY name`).all();
  return c.json(rows.results || []);
});
...
app.delete("/api/items/:id", async (c) => {
  const id = Number(c.req.param("id"));
  if (!Number.isFinite(id)) return jsonError("invalid item id", 400);
  const inUse = await c.env.DB.prepare(`SELECT COUNT(*) AS c FROM expenses WHERE item_id = ?`).bind(id).first();
...
app.get("/api/expenses/check-duplicate", async (c) => {
  ...
  if (itemIDStr) {
    ...
    const row = excludeID ? await c.env.DB.prepare(
      `SELECT COUNT(*) AS c FROM expenses WHERE item_id = ? AND date = ? AND id != ?`
    ).bind(itemID, date, excludeID).first() : await c.env.DB.prepare(`SELECT COUNT(*) AS c FROM expenses WHERE item_id = ? AND date = ?`).bind(itemID, date).first();
    count = row?.c ?? 0;
  } else if (name) {
    const row = excludeID ? await c.env.DB.prepare(
    ...
    ).bind(name, date, excludeID).first() : await c.env.DB.prepare(
      `SELECT COUNT(*) AS c FROM expenses e
           JOIN items i ON i.id = e.item_id
...
app.get("/api/expenses", async (c) => {
  ...
  const expenses = (rows.results || []).map((row) => ({
    id: row.id,
    person_id: row.person_id,
    person_name: row.person_name,
    shop_id: row.shop_id,
...
app.post("/api/expenses", async (c) => {
  ...
  for (const item of items) {
    ...
    ).bind(personId, shopId, itemId, date, Number(item.amount)).run();
...
app.put("/api/expenses/:id", async (c) => {
  ...
  const shop = await c.env.DB.prepare(`SELECT id FROM shops WHERE id = ?`).bind(shopId).first();
  if (!shop) return jsonError("invalid shop_id", 400);
  const existing = await c.env.DB.prepare(`SELECT id FROM expenses WHERE id = ?`).bind(id).first();
  if (!existing) return jsonError("expense not found", 404);
  const itemId = await upsertItem(c.env.DB, name);
  await c.env.DB.prepare(
    `UPDATE expenses SET person_id = ?, shop_id = ?, item_id = ?, date = ?, amount = ? WHERE id = ?`
  ).bind(personId, shopId, itemId, date, amount, id).run();
